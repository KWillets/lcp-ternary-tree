...
public:
  ...
  void graph( int parent_id, char *portpos );
};
...
void StringNode::graph( int parent_id, char *portpos ) {
  int my_id = max_id;
  max_id += 2;
  printf( "Node%d[label=\"%s\" shape=rect ];\n", my_id, pivot );
  if( parent_id >=0) printf(" Node%d->Node%d;\n", parent_id, my_id );
  if( left ) left->graph( my_id, "sw" );
  if( right ) right->graph( my_id, "se"  );
}

template<bool ascending>
void LcpNode<ascending>::graph( int parent_id, char *portpos ) {
  int my_id = max_id;
  max_id += 4;
